...
class _VoiceTabState extends ConsumerState<VoiceTab> {
          ...
          SnackBar(
            content: Text(
              e.isInsufficientTokens
                  ? 'Not enough tokens to regenerate voice'
                  : e.message,
            ),
          ),
        );
  ...
  Widget build(BuildContext context) {
    ...
    return compositionAsync.when(
      data: (composition) {
        final voicesAsync = ref.watch(
          voicesProviderForLanguage(composition.language),
        );
        return ListView(
            ...
            if (composition.voiceoverUrl != null)
              Card(
                child: ListTile(
                  ...
                  onTap: () =>
                      _toggleVoiceoverPlayback(composition.voiceoverUrl!),
                ),
              ),
...
final voicesProviderForLanguage = FutureProvider.autoDispose.family((
  ref,
  String language,
) async {
